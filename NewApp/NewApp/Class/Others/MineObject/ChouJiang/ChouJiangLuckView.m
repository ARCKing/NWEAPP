...
// 链接
#define GiftURL @"http://zqw.2662126.com/App/mission/draw_data"
#define GiftNumber @"http://zqw.2662126.com/App/mission/turntable"

#define GiftUserList @"http://zqw.2662126.com/App/mission/draw_list"
...
@property(nonatomic,strong)NSString * gift_message;
...
//     _scoresl.text = [NSString stringWithFormat:@"剩余抽奖次数:%@",_number];
    ...
    NSAttributedString * att = [self addAppandRootViewAttributedText:@"剩余抽奖次数:" andArticleNum:_number andColor1:[UIColor blackColor] andColor2:[UIColor redColor]];
    
    _scoresl.attributedText = att;
    [_scoresl setFont:[UIFont fontWithName:@"Zapfino" size:16.0]];
            ...
            if (![responseObject[@"data"] isEqual:[NSNull null]] && responseObject[@"data" ] != nil) {
                ...
                if ((int)data.count > 0) {
                    for (NSDictionary * dic in data) {
                        ...
                        model.thumb = [NSString stringWithFormat:@"%@",dic[@"thumb"]];
    ...
    [manger POST:GiftNumber parameters:_userinfo progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([code isEqualToString:@"1"]){
            ...
            if (![responseObject[@"data"] isEqual:[NSNull null]] && responseObject[@"data" ] != nil) {
                ...
                self.gift_price = [NSString stringWithFormat:@"%@",dic[@"price"]];
...
- (void)createUI
{
    ...
    // 背景图
    _bgImgv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT - 64)];
    _bgImgv.image = [UIImage imageNamed:@"bg_prizes"];
    ...
    /*
//    // top
    UIImageView *topImgv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT/4)];
    topImgv.image = [UIImage imageNamed:@"bg_prize_top"];
...
- (void)nineSquareView
{
    ...
    // 九宫格底图
    _ninebgImgv = [[UIImageView alloc] initWithFrame:CGRectMake(0, HEIGHT - 64 - WIDTH - WIDTH/15, WIDTH, WIDTH)];
//    _ninebgImgv.image = [UIImage imageNamed:@"sprize"];
    _ninebgImgv.userInteractionEnabled = YES;
    [_bgImgv addSubview:_ninebgImgv];
    
    _ninebgImgv.backgroundColor = [UIColor clearColor];
...
//            [btn setImage:[UIImage imageNamed:@"ic_select"] forState:UIControlStateNormal];
            
            [btn setBackgroundImage:[UIImage imageNamed:@"ic_select"] forState:UIControlStateNormal];
...
#pragma mark- 抽奖按钮
            ...
            if (!_readyb)
            {
                ...
                [_readyb setBackgroundImage:[UIImage imageNamed:@"ic_default_award"] forState:UIControlStateNormal];
...
- (void)startedAction
{
    ...
    self.giftStBc = ^{
        ...
        [weakSelf.readyb setBackgroundImage:[UIImage imageNamed:@"ic_await_award"] forState:UIControlStateNormal];
...
- (void)giftAleartViewShow:(BOOL)end{
    ...
    if (self.giftAleartView == nil) {
        ...
        self.giftAleartView.image = [UIImage imageNamed:@"bg_prize_dialogs"];
